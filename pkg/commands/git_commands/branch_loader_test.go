...
func TestSelectBehindForBranch(t *testing.T) {
	...
	scenarios := []scenario{
		...
		},
		{
			...
			// Today's code may pick master (behind=0); new code picks develop (behind=5).
			aheadBehinds: []aheadBehind{
				{ahead: 55, behind: 0, valid: true}, // master
				{ahead: 5, behind: 5, valid: true},  // develop  ← smallest ahead, MORE correct
			},
			expected: 5,
		},
		{
	...
	for _, s := range scenarios {
		t.Run(s.testName, func(t *testing.T) {
			result := selectBehindIdxForBranch(s.aheadBehinds)
			assert.Equal(t, s.expected, result)
		})
	}
}

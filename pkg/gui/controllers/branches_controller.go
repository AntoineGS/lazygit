...
func NewBranchesController(
	...
	chord.RegisterTitleFunc("localBranches", "r", helpers.RebasingTitle(c.HelperCommon, true))

	chord.RegisterTitleFunc("localBranches", "d", func() string {
		if c.Git() == nil {
			return c.Tr.DeleteLocalBranch
		}
		...
		if len(items) > 1 {
			return c.Tr.DeleteBranchesTitle
		}
		return utils.ResolvePlaceholderString(
			c.Tr.DeleteBranchTitle,
			map[string]string{"selectedBranchName": items[0].Name},
		)
	})

	upstreamShortName := func() string {
		if c.Git() == nil {
			return ""
		}
		...
		if sel.RemoteBranchStoredLocally() {
			return sel.ShortUpstreamRefName()
		}
		return c.Tr.UpstreamGenericName
	}

	chord.RegisterTitleFunc("localBranches", "ug", func() string {
		upstream := upstreamShortName()
		if upstream == "" {
			return c.Tr.ViewUpstreamResetOptions
		}
		return c.Tr.ResetTo + " " + upstream
	})

	chord.RegisterTitleFunc("localBranches", "ur", helpers.RebasingTitle(c.HelperCommon, false))

	chord.RegisterTitleFunc("localBranches", "g", helpers.ResetToRefTitle(c.HelperCommon, c.Tr.ViewResetOptions, func() (string, bool) {
		sel := ctrl.context().GetSelected()
		if sel == nil {
			return "", false
		}
		return sel.RefName(), true
	}))

	chord.RegisterTitleFunc("localBranches", "M", func() string {
		return c.Tr.Merge
	})
...
func (self *BranchesController) GetKeybindings(opts types.KeybindingsOpts) []*types.Binding {
	return []*types.Binding{
		...
		},
		{
			...
			Tooltip:         self.c.Tr.DeleteLocalBranchTooltip,
		},
		{
		...
		},
		{
			...
			Tooltip:         self.c.Tr.DeleteLocalAndRemoteBranchTooltip,
		},
		{
		...
		},
		{
			...
			TooltipFunc: func() string {
				...
				if len(self.c.Model().Branches) > 0 {
					checked = self.c.Model().Branches[0].Name
				}
		...
		},
		{
			...
			GetDisabledReason: func() *types.DisabledReason {
				if reason := self.require(self.singleItemSelected(self.notMergingIntoYourself, self.nonFastForwardMergeApplicable))(); reason != nil {
					return reason
				}
				ref := self.selectedBranchShortName()
				if !self.regularMergeWillFastForward(ref) {
			...
			HiddenInChordPopup: func() bool {
				return !self.regularMergeWillFastForward(self.selectedBranchShortName())
			},
		},
		{
			...
			GetDisabledReason: func() *types.DisabledReason {
				if reason := self.require(self.singleItemSelected(self.notMergingIntoYourself, self.fastForwardOnlyMergeApplicable))(); reason != nil {
					return reason
				}
				ref := self.selectedBranchShortName()
				if self.regularMergeWillFastForward(ref) {
		...
		},
		{
			...
			TooltipFunc: func() string {
				...
				return utils.ResolvePlaceholderString(
					...
					map[string]string{"selectedBranch": ref},
				)
			},
		},
		{
...
// regularMergeWillFastForward reports whether a "regular" merge of refName
// into the checked-out branch will fast-forward, based on user/git config
// + branch state. Mirrors the predicate the old MergeRefIntoCheckedOutBranch
// used to decide which variant to show in slot 1 of the merge menu.
func (self *BranchesController) regularMergeWillFastForward(refName string) bool {
	if self.c.Git() == nil {
		return false
	}
...
func (self *BranchesController) gitMixedResetToUpstream(branch *models.Branch) error {
	return self.c.Helpers().Refs.PerformGitReset(branch.FullUpstreamRefName(), "mixed")
}

func (self *BranchesController) gitSoftResetToUpstream(branch *models.Branch) error {
	return self.c.Helpers().Refs.PerformGitReset(branch.FullUpstreamRefName(), "soft")
}

func (self *BranchesController) gitHardResetToUpstream(branch *models.Branch) error {
	return self.c.Helpers().Refs.PerformGitReset(branch.FullUpstreamRefName(), "hard")
}
...
func (self *BranchesController) notMergingIntoYourself(branch *models.Branch) *types.DisabledReason {
	return self.c.Helpers().MergeAndRebase.MergeIntoSelfDisabledReason(branch.Name)
}

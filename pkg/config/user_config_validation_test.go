...
func TestKeybindingGroupsValidation_DuplicateGroupNamesAreRejected(t *testing.T) {
	cfg := GetDefaultConfig()
	// Use chord heads that don't clash with any default Files leaf
	// binding so the duplicate-name check is what fires.
	cfg.Keybinding.Files.RefreshFiles = "ez"
	cfg.Keybinding.Universal.Pull = "fp"
	cfg.KeybindingGroups["files"] = map[string]KeybindingGroupConfig{
		"e": {Name: "Same name"},
		"f": {Name: "Same name"},
	}
...
func TestKeybinding_ChordWithModifierInMiddleRejected(t *testing.T) {
	cfg := GetDefaultConfig()
	cfg.Keybinding.Universal.Pull = "b<ctrl+p>"

	err := cfg.Validate()
	if err == nil {
		t.Fatal("expected validation error for modifier in middle of chord")
	}
...
func TestKeybinding_ChordWithModifierAtStartAllowed(t *testing.T) {
	cfg := GetDefaultConfig()
	// A modifier on the first chord key is fine; only middle/tail
	// positions are rejected.
	cfg.Keybinding.Universal.Pull = "<ctrl+b>p"

	if err := cfg.Validate(); err != nil {
		t.Fatalf("modifier-on-first chord key should validate, got: %v", err)
	}
}

func TestKeybindingGroup_PrefixWithModifierInMiddleRejected(t *testing.T) {
	cfg := GetDefaultConfig()
	cfg.Keybinding.Universal.Pull = "b<ctrl+p>x"
	cfg.KeybindingGroups = map[string]map[string]KeybindingGroupConfig{
		"global": {
			"b<ctrl+p>": {Name: "Bad chord"},
		},
	}

	err := cfg.Validate()
	if err == nil {
		t.Fatal("expected validation error for modifier in middle of chord prefix")
	}
	if !strings.Contains(err.Error(), "modifier") {
		t.Errorf("error should mention modifier, got: %v", err)
	}
...
func TestKeybindingGroup_LeafCollisionRejected_FilesContext(t *testing.T) {
	// Catches the case where a user customizes a non-legacy leaf binding
	// in the same KeybindingConfig section that owns the chord prefix.
	cfg := GetDefaultConfig()
	cfg.Keybinding.Files.RefreshFiles = "S"

	err := cfg.Validate()
	if err == nil {
		t.Fatal("expected validation error for files chord/leaf collision")
	}
	if !strings.Contains(err.Error(), "S") {
		t.Errorf("error should cite the colliding key, got: %v", err)
	}
}

func TestKeybindingGroup_LeafCollision_OtherSectionAllowed(t *testing.T) {
	// Files chord prefix "S" must coexist with a same-key leaf in a
	// different KeybindingConfig section (different view at runtime).
	cfg := GetDefaultConfig()
	cfg.Keybinding.Commits.RevertCommit = "S"

	if err := cfg.Validate(); err != nil {
		t.Fatalf("cross-section key reuse must not error; got: %v", err)
	}
}

func TestKeybindingGroup_LeafCollision_UniversalNotFlaggedForNonGlobal(t *testing.T) {
	// Universal.Remove="d" coexists with files chord prefix "d" in
	// defaults; chord-dispatch wins at runtime and the legacy single-key
	// leaf is intentionally shadowed. Validating that case as an error
	// would break the default config.
	cfg := GetDefaultConfig()
	if err := cfg.Validate(); err != nil {

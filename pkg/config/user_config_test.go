...
func TestKeybindingGroups_UnmarshalYAML_NestedPrefixNamedName(t *testing.T) {
	// Pathological but legal: a chord prefix label is literally "name".
	// The old key-name heuristic misclassified this as flat; the
	// value-shape heuristic correctly classifies it as nested.
	yamlInput := `
keybindingGroups:
  files:
    name: { name: "Names" }
`
	cfg := &UserConfig{}
	assert.NoError(t, yaml.Unmarshal([]byte(yamlInput), cfg))
	assert.Equal(t, "Names", cfg.KeybindingGroups["files"]["name"].Name)
	_, leakedToGlobal := cfg.KeybindingGroups["global"]
	assert.False(t, leakedToGlobal, "nested shape with prefix=='name' must not be misclassified as flat")
}

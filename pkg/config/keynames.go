...
// CanonicalizePrefixLabel normalises a chord-prefix label so equivalent
// spellings (e.g. "<b><t>" and "bt") resolve to the same value. Returns
// the input unchanged on parse failure.
func CanonicalizePrefixLabel(label string) string {
...
// NormalizeGroupKeys applies CanonicalizePrefixLabel to every prefix
// label in a per-context KeybindingGroups map.
func NormalizeGroupKeys(in map[string]KeybindingGroupConfig) map[string]KeybindingGroupConfig {
	out := make(map[string]KeybindingGroupConfig, len(in))
	for label, g := range in {
		out[CanonicalizePrefixLabel(label)] = g
	}
	return out
}

...
var ChordUpstreamRebasePopup = NewIntegrationTest(NewIntegrationTestArgs{
	...
	Run: func(t *TestDriver, keys config.KeybindingConfig) {
		...
		t.Views().Branches().Press(keys.ChordPrefix.Get("localBranches", config.ChordIDBranchUpstreamOptions))
		t.ExpectPopup().Menu().Title(Equals("Upstream options"))

		t.GlobalPress("r")
		t.ExpectPopup().Menu().
			Title(Equals("Rebase 'master'")).

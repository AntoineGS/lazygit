...
func TestWithRest_MutatingInputDoesNotAffectStored(t *testing.T) {
	g := NewKeyRune('g')
	p := NewKeyRune('p')
	x := NewKeyRune('x')
	z := NewKeyRune('z')
	...
	rest := []Key{p, x}
	chord := g.WithRest(rest)

	// Mutate the caller's slice after WithRest. The stored chord must
	// not observe this change.
	rest[0] = z
	...
	if !seq[1].Equals(p) {
		t.Errorf("Sequence()[1] should still equal original 'p'; got %+v", seq[1])
	}
	if !seq[2].Equals(x) {
		t.Errorf("Sequence()[2] should still equal original 'x'; got %+v", seq[2])
	}
}
